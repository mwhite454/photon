...
function updateLockedPathNotes() {
    if (processed.model && processed.lockedPath) {
        ...
        const endpoints = photon.point.fromPathEnds(pathAndOffset.result, pathAndOffset.offset);
        if (pathAndOffset) {
            setNotes([
...
function measureLockedPath() {
    ...
    if (!pathAndOffset)
        return null;
    var measure = photon.measure.pathExtents(pathAndOffset.result);
...
function isMeasurementEqual(m1, m2) {
    ...
    if (!m1 || !m2)
        return false;
    if (!photon.measure.isPointEqual(m1.low, m2.low))
        return false;
    if (!photon.measure.isPointEqual(m1.high, m2.high))
        return false;
    return true;
}
export function setProcessedModel(model, error) {
    ...
    if (!photon.maker.isPoint(processed.model.origin))
...
//public members
export var onInit;
export var onViewportChange;
export var fullScreen;
export var dockMode;
export var codeMirrorEditor;
export var codeMirrorOptions = {
    extraKeys: {
...
export var viewScale;
export var querystringParams;
export var pointers;
export var useWorkerThreads = true;
export var mainThreadConstructor;
export var fontDir;
export function runCodeFromEditor(paramValues) {
...
export function processResult(value) {
    ...
    if (typeof result === 'function') {
        ...
        if (useWorkerThreads && Worker) {
            constructInWorker(codeMirrorEditor.getDoc().getValue(), value.orderedDependencies, function (model) {
                enableKit();
...
export function setParam(index, value) {
    clearTimeout(setParamTimeoutId);
    setParamTimeoutId = setTimeout(function () {
        throttledSetParam(index, value);
    }, 50);
}
export function animate(paramIndex = 0, milliSeconds = 150, steps = 20) {
...
export function activateParam(input, onLongHold = false) {
    function activate() {
...
export function deActivateParam(input, delay) {
...
export function filenameFromRequireId(id, bustCache) {
...
export function downloadScript(url, callback) {
...
export function toggleClassAndResize(name) {
    toggleClass(name);
    onWindowResize();
}
export function toggleClass(name, element = document.body) {
...
export function downloadClick(a, format) {
    //show options
    ...
    try {
        switch (request.format) {
            ...
            case ExportFormat.SvgPathData:
                text = photon.exporter.toSVGPathData(processed.model, request.options);
                break;
            default:
                return false;
        }
...
export function command(cmd, value) {
    switch (cmd) {
        case "insert":
            return setTimeout(() => {
...
function initializePlayground() {
    ...
    codeMirrorEditor = MonacoEditorAdapter.createEditor(function (elt) {
    ...
    var parentLoad = querystringParams['parentload'];
    if (parentLoad) {
