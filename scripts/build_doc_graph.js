  ...
  for (const line of fmLines) {
    ...
    if (m) {
      ...
      if (val.startsWith('[') && val.endsWith(']')) {
        try { fm[key] = JSON.parse(val.replace(/'/g, '"')); } catch(e) { fm[key] = val; }
      } else {
...
function main() {
  ...
  for (const file of files) {
    ...
    nodes.push({ id: meta.id, title: meta.title, path: path.relative(process.cwd(), file), summary: meta.summary, tags: meta.tags });
  }
  ...
  for (const file of files) {
    ...
    for (const l of links) {
      ...
      const type = /example|demo|play|try/.test(text) ? 'example-of' : 'references';
  ...
  console.log('Wrote', OUT, 'with', nodes.length, 'nodes and', edges.length, 'edges');
}
