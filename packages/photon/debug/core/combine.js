import { cloneObject, extendObject, pathType, createRouteKey } from './maker.js';
...
function getPointsOnPath(points, onPath, popOptions) {
    ...
    points.forEach(p => {
        if (isPointOnPath(p, onPath, 0.00001, undefined, popOptions)) {
...
function breakAlongForeignPath(crossedPath, overlappedSegments, foreignWalkedPath) {
    ...
    const foreignIntersection = (_a = pathInternal.intersection) === null || _a === void 0 ? void 0 : _a.call(pathInternal, crossedPath.pathContext, foreignPath, options);
...
export function isPathInsideModel(pathContext, modelContext, pathOffset, farPoint, measureAtlas) {
    const options = { farPoint };
    if (measureAtlas) {
            ...
            const walkModelToIntersectOptions = {
                onPath(innerWalkedPath) {
                    if (outerWalkedPath.pathContext !== innerWalkedPath.pathContext &&
                        measure.isMeasurementOverlapping(modelToBreakAtlas.pathMap[outerWalkedPath.routeKey], modelToIntersectAtlas.pathMap[innerWalkedPath.routeKey])) {
...
export function breakPathsAtIntersections(modelToBreak, modelToIntersect) {
    const modelToBreakAtlas = new measure.Atlas(modelToBreak);
    modelToBreakAtlas.measureModels();
    let modelToIntersectAtlas;
    if (!modelToIntersect) {
...
export function combine(modelA, modelB, includeAInsideB = false, includeAOutsideB = true, includeBInsideA = false, includeBOutsideA = true, options) {
    const opts = {
        trimDeadEnds: true,
    ...
    if (!opts.farPoint) {
        const measureBoth = measure.increase(measure.increase({ high: [null, null], low: [null, null] }, opts.measureA.modelMap['']), opts.measureB.modelMap['']);
...
export function combineIntersection(modelA, modelB) {
    return combine(modelA, modelB, true, false, true, false);
}
export function combineSubtraction(modelA, modelB) {
    return combine(modelA, modelB, false, true, true, false);
}
export function combineUnion(modelA, modelB) {
