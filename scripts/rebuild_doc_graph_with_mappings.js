...
function readFrontmatterAndBody(content) {
  ...
  let end = -1;
  for (let i = 1; i < parts.length; i++) if (parts[i].trim() === '---') { end = i; break; }
...
function inferLinksFromBody(body, fromPath) {
  ...
  while ((m = mdRegex.exec(body)) !== null) {
    ...
    if (href.startsWith('../') || href.endsWith('.md') || href.startsWith('./')) {
      // resolve relative
      const resolved = path.resolve(path.dirname(fromPath), href.split('#')[0]);
      if (fs.existsSync(resolved) && resolved.endsWith('.md')) links.push({ text, href, mapped: resolved });
    }
  }
  return links;
}
...
function buildGraph() {
  ...
  for (const file of files) {
    ...
    const id = fm.id || ('makerjs.' + path.relative(ROOT, file).replace(/\\/g, '/').replace(/\.md$/i, '').replace(/\//g, '.'));
    ...
    nodes.push({ id, title, path: path.relative(process.cwd(), file), summary, tags: fm.tags || [] });
  }
  ...
  for (const file of files) {
    const raw = fs.readFileSync(file, 'utf8');
    const { fm, body } = readFrontmatterAndBody(raw);
    const fromId = fm.id || ('makerjs.' + path.relative(ROOT, file).replace(/\\/g, '/').replace(/\.md$/i, '').replace(/\//g, '.'));
    const links = inferLinksFromBody(body, file);
    for (const l of links) {
      const mapped = l.mapped;
      if (!mapped) continue;
      const targetPath = path.resolve(mapped);
      const targetId = pathToId[targetPath] || ('makerjs.' + path.relative(ROOT, mapped).replace(/\\/g, '/').replace(/\.md$/i, '').replace(/\//g, '.'));
      const type = /example|demo|play|try/i.test(l.text) ? 'example-of' : 'references';
      edges.push({ from: fromId, to: targetId, type, text: l.text, href: l.href });
    }
  }
...
function main() {
  ...
  console.log('Wrote', OUT, 'with', graph.nodes.length, 'nodes and', graph.edges.length, 'edges. Backup at', BACKUP);
}
