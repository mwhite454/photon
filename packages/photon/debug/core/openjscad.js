...
(function (MakerJs) {
    ...
    (function (exporter) {
        ...
        function chainToJscadScript(chainContext, facetSize, accuracy) {
            ...
            var beginMap = {};
            beginMap[pathType.Circle] = function (circle, link) {
                var circleOptions = {
                    center: point.rounded(point.add(circle.origin, link.walkedPath.offset), accuracy),
            ...
            beginMap[pathType.Line] = function (line, link) {
            ...
            beginMap[pathType.Arc] = function (arc, link) {
                var endAngle = angle.ofArcEnd(arc);
                if (link.reversed) {
                    reverseTail = true;
                }
                var arcOptions = {
                    center: point.rounded(point.add(arc.origin, link.walkedPath.offset), accuracy),
            ...
            var appendMap = {};
            appendMap[pathType.Line] = function (line, link) {
            ...
            appendMap[pathType.Arc] = function (arc, link) {
                var reverse = (reverseTail != link.reversed);
                var endAngle = angle.ofArcEnd(arc);
                var arcOptions = {
                    radius: round(arc.radius, accuracy),
        ...
        function convertChainsTo2D(convertToT, union, subtraction, modelToExport, jsCadCagOptions = {}) {
            ...
            function addChains(layerId, cs) {
                cs.forEach(c => {
                    if (!c.endless)
                        return;
                    const add = { cag: convertToT(c, jsCadCagOptions.maxArcFacet), subtracts: [] };
                    if (c.contains) {
        ...
        function convert2Dto3D(to2D, to3D, union3D, modelToExport, options = {}) {
            ...
            if (options.byLayers) {
                ...
                let csg = to3D(result2D, options.extrude, options.z);
                csgs.push(csg);
            }
...
//# sourceMappingURL=openjscad.js.map