...
(function (MakerJs) {
    ...
    (function (models) {
        ...
        function getLargestArc(b, startT, endT, accuracy) {
            ...
            var line = new paths.Line(start.point, test.point);
            line.bezierData = {
                startT: startT,
                endT: test.t
            };
        ...
        function getChainBezierRange(curve, c, layer, addToLayer, pointMatchingDistance) {
            ...
            var actualBezierRanges = endLinks.map(endLink => getActualBezierRange(curve, endLink.walkedPath.pathContext, endLink.endPoints, endLink.walkedPath.offset, pointMatchingDistance));
            var result = {
        ...
        class BezierCurve {
            ...
            static getBezierSeeds(curve, options = {}) {
                ...
                model.findChains(curve, function (chains, loose, layer) {
                    chains.forEach(c => {
                    ...
                    loose.forEach(wp => {
                        ...
                        var range = getActualBezierRange(curve, wp.pathContext, point.fromPathEnds(wp.pathContext), wp.offset, options.pointMatchingDistance);
                        if (range) {
                            var b = getScratch(curve.seed);
                            var piece = b.split(range.startT, range.endT);
                            addToLayer(BezierToSeed(piece), layer);
